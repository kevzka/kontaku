                                              ...
                                              buttonContactDetails(
                                                ...
                                                onPressed: () async {
                                                  ...
                                                  if (result == "true") {
                                                    ...
                                                    context.pop(
                                                      true,
                                                    ); // Return true to refresh parent list
                                                  } else if(result == "number_changed") {
                                                    await Kontaku.snackbarNotification(
                                                      context,
                                                      "Nomor berhasil diubah. Perubahan lainnya juga disimpan.",
                                                    );
                                                    context.go('/mainNavigation/0'); // Go to main navigation to refresh list
                                                  } else {
                                                    ScaffoldMessenger.of(
                                                      context,
                                                    ).showSnackBar(
                              ...
                              children: [
                                ...
                                SizedBox(
                                  ...
                                  child: TextButton(
                                    onPressed: (){
                                      _closeDeleteDialog();
                                      context.pop(); // Close edit screen without saving
                                    },
...
Future<String> updateContactDetails({
  required String uid,
  ...
  print(
    'Updating contact details for uid:$uid number:$number with name:$name email:$email notes:$notes',
  );
  try {
    final firestore = FirebaseFirestore.instance;

    if (number != numberOri) {
      // 1. Referensi dokumen lama
      ...
      if (oldDocSnap.exists) {
        // Ambil semua data lama ke dalam Map
        final oldData = Map<String, dynamic>.from(oldDocSnap.data()!);

        // Tambahkan / ganti data lama dengan data baru yang ingin di-update
        oldData['name'] = name;
        oldData['email'] = email;
        oldData['notes'] = notes;
        oldData['number'] = number; // Update nomor dengan yang baru

        // 2. Referensi dokumen baru
        ...
        // Gunakan Batch agar proses buat baru & hapus lama terjadi bersamaan (Atomik)
        final batch = firestore.batch();

        // Set dokumen baru dengan data yang sudah digabung
        batch.set(newDocRef, oldData);

        // 3. Hapus dokumen lama
        batch.delete(oldDocRef);

        // Eksekusi semuanya sekaligus
        await batch.commit();
        print('Kontak berhasil diperbarui dan dipindahkan!');
      } else {
        print(
          'No existing contact found with number: $numberOri for uid: $uid',
        );
      }
      ...
      final contactRef = FirebaseFirestore.instance
          ...
          .collection('contacts')
          .doc(numberOri);
      contactRef.update({'name': name, 'email': email, 'notes': notes});
    }
    return "true";
  } catch (e) {
